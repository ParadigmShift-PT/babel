...
public class InConnectionHandler<T> extends ConnectionHandler<T> {
    ...
    protected boolean outsideUp;
    protected final InConnListener<T> listener;

    public InConnectionHandler(long connectionId, InConnListener<T> listener,
                               MessageListener<T> consumer, EventLoop loop, Attributes selfAttrs,
                               MessageEncoder<T> encoder, MessageDecoder<T> decoder) {
        super(connectionId, consumer, loop, true, selfAttrs);
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        ...
        cause.printStackTrace(); // TODO debug. remove
        if (outsideUp) {
